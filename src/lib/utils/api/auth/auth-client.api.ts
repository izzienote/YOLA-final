...
export const fetchUserSessionState = async (): Promise<{
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_BASE_URL}/api/auth/user-session-state`, {
      method: 'GET',
      credentials: 'include'
    });
...
export const fetchDuplicateCheck = async (field: string, value: string): Promise<boolean> => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_BASE_URL}${API.DUPLICATE}?field=${field}&value=${encodeURIComponent(value)}`,
      {
        method: 'GET',
        headers: {
